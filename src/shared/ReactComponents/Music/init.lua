...
local function Music(props: { PlayerData: PlayerData })
	...
	local currentEvent, setCurrentEvent = React.useState("none")
	...
	local Volume, setVolume = React.useState(0.2)
	...
	useEffect(function()
		...
		task.spawn(function()
			...
			connection = SpecialEvents.OnClientEvent:Connect(function(events)
	...
	-- Load music based on current event
	useEffect(function()
		local soundtracksFolder = game.ReplicatedStorage.Shared:FindFirstChild("Soundtracks")
		local list = {}

		if soundtracksFolder then
			local eventFolder = soundtracksFolder:FindFirstChild(currentEvent)
			if eventFolder then
				for _, inst in eventFolder:GetChildren() do
		...
		-- Fallback to default music if no event-specific music found
		if #list == 0 then
			local musicFolder = game.ReplicatedStorage.Shared:FindFirstChild("Music")
			if musicFolder then
				for _, inst in musicFolder:GetDescendants() do
					if inst:IsA("Sound") then
						table.insert(list, inst)
					end
				end
		...
		setMusics(list)
		setCurrentIndex(1) -- Reset to first track when music list changes
	end, { currentEvent })

	-- Subscribe to PlayerSessions for settings
	useEffect(function()
		local player = game.Players.LocalPlayer

		local unsubscribe = PlayerSessions:subscribe(function(state)
			local playerEntity = state.players[player]
			if playerEntity and playerEntity.Data and playerEntity.Data.PlayerSettings then
				local settings = playerEntity.Data.PlayerSettings
	...
	useEffect(function()
		...
		task.spawn(function()
			...
			if props.MusicOpen then
				...
				-- 	:WaitForChild("SetPlayerSettings")
				-- 	:FireServer({ MusicVolume = Volume })
	...
	return e("Frame", {
		...
		MainLayout = e("Frame", {
			...
			EventLabel = currentEvent ~= "none" and e("TextLabel", {
				Position = UDim2.new(0.5, 0, 0.5, 0),
				AutomaticSize = Enum.AutomaticSize.XY,
				AnchorPoint = Vector2.new(0.5, 0.5),
				BackgroundColor3 = SpecialEventsState.getEventColor(currentEvent),
				Text = SpecialEventsState.getEventName(currentEvent) or "Special Event",
				TextSize = 20,
